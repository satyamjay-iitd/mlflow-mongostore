...
class TestMongoStore(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        for model in (MongoRun, MongoExperiment, SequenceId, MongoMetric):
    ...
    def test_search_experiments_filter_by_attribute(self):
        ...
        experiments = self.store.search_experiments(
            filter_string="attribute.name = 'a'"
        )
        assert [e.name for e in experiments] == ["a"]
        experiments = self.store.search_experiments(
            filter_string="attribute.`name` = 'a'"
    ...
    def test_search_experiments_filter_by_time_attribute(self):
        ...
        experiments = self.store.search_experiments(
            filter_string=f"creation_time >= {now}"
        )
        assert experiments == []
    ...
    def test_search_experiments_filter_by_tag(self):
        ...
        experiments = self.store.search_experiments(
            filter_string="tag.`k e y 1` = 'value'"
        )
        ...
        experiments = self.store.search_experiments(
            filter_string="tag.key1 LIKE 'val%'"
        )
        assert [e.name for e in experiments] == ["exp1"]
        experiments = self.store.search_experiments(
            filter_string="tag.key1 LIKE '%Lue'"
        )
        assert [e.name for e in experiments] == ["exp2"]
        experiments = self.store.search_experiments(
            filter_string="tag.key1 ILIKE '%alu%'"
        )
        assert [e.name for e in experiments] == ["exp2", "exp1"]
        experiments = self.store.search_experiments(
    ...
    def test_search_experiments_max_results_validation(self):
        with pytest.raises(
            MlflowException, match=r"It must be a positive integer, but got None"
        ):
            self.store.search_experiments(max_results=None)
        with pytest.raises(
            MlflowException, match=r"It must be a positive integer, but got 0"
        ):
            self.store.search_experiments(max_results=0)
        with pytest.raises(
            MlflowException, match=r"It must be at most \d+, but got 1000000"
        ):
    ...
    def test_search_experiments_pagination(self):
        ...
        experiments = self.store.search_experiments(
            max_results=4, page_token=experiments.token
        )
        ...
        assert [e.name for e in experiments] == reversed_experiment_names[8:] + [
            "Default"
        ]
    ...
    def test_set_experiment_tag(self):
        ...
        multi_line_Tag = entities.ExperimentTag(
            "multiline tag", "value2\nvalue2\nvalue2"
        )
    ...
    def test_delete_restore_experiment_with_runs(self):
        experiment_id = self._experiment_factory("test exp")
        run1 = self._run_factory(
            config=self._get_run_configs(experiment_id)
        ).info.run_id
        run2 = self._run_factory(
            config=self._get_run_configs(experiment_id)
        ).info.run_id
        ...
        for deleted_run in deleted_run_list:
            ...
            assert self.store._get_run(deleted_run.info.run_id).deleted_time is not None
    ...
    def test_log_metric_concurrent_logging_succeeds(self):
        ...
        for run in [run1, run2, run1, run2]:
            ...
            assert (
                len(self.store.get_metric_history(run.info.run_id, "metric_key")) >= 100
            )
            for batch_idx in range(5):
                assert (
                    len(
    ...
    def test_update_run_name(self):
        ...
        self.store.set_tag(
            run_id, entities.RunTag(mlflow_tags.MLFLOW_RUN_NAME, "newest name")
        )
        run = self.store.get_run(run_id)
    ...
    def test_order_by_metric_tag_param(self):
        ...
        assert self.get_ordered_runs(
            ["metrics.x asc", "metrics.y asc"], experiment_id
        ) == [
            "-inf/4",
            "-1000/5",
            "0/6",
        ...
        assert self.get_ordered_runs(
            ["metrics.x asc", "tag.metric asc"], experiment_id
        ) == [
            "-inf/4",
            "-1000/5",
            "0/6",
        ...
        # asc/desc
        assert self.get_ordered_runs(
            ["metrics.x asc", "metrics.y desc"], experiment_id
        ) == [
            "-inf/4",
            "-1000/5",
            "0/7",
        ...
        assert self.get_ordered_runs(
            ["metrics.x asc", "tag.metric desc"], experiment_id
        ) == [
            "-inf/4",
            "-1000/5",
            "0/7",
        ...
        # desc / asc
        assert self.get_ordered_runs(
            ["metrics.x desc", "metrics.y asc"], experiment_id
        ) == [
            "inf/3",
            "1000/8",
            "0/6",
    ...
    def test_search_tags(self):
        ...
        # test search returns appropriate run (same key different values per run)
        assert self._search(
            experiment_id, filter_string="tags.generic_2 = 'some value'"
        ) == [r1]
        assert self._search(
        ...
        assert self._search(
            experiment_id, filter_string="tags.generic_2 LIKE '%other%'"
        ) == [r2]
        assert (
        ...
        assert self._search(
            experiment_id, filter_string="tags.generic_2 ILIKE '%Other%'"
        ) == [r2]
        assert self._search(
            experiment_id,
    ...
    def test_search_attrs(self):
        ...
        filter_string = "attribute.status = '{}'".format(
        ...
        if is_windows():
            ...
            expected_artifact_uri = (
                pathlib.Path.cwd()
                .joinpath(ARTIFACT_URI, e1, r1, "artifacts")
                .as_posix()
            )
            filter_string = f"attr.artifact_uri = '{expected_artifact_uri}'"
        assert self._search([e1, e2], filter_string) == [r1]
    ...
    def test_search_runs_run_name(self):
        ...
        run1 = self._run_factory(
            dict(self._get_run_configs(exp_id), run_name="run_name1")
        )
        run2 = self._run_factory(
            dict(self._get_run_configs(exp_id), run_name="run_name2")
        )
        result = self.store.search_runs(
            [exp_id],
    ...
    def test_log_batch_param_overwrite_disallowed(self):
        ...
        self._verify_logged(
            self.store, run.info.run_id, metrics=[metric], params=[param], tags=[]
        )
    ...
    def test_log_batch_params_max_length_value(self):
        ...
        self._verify_logged(
            self.store, run.info.run_id, [], expected_param_entities, []
        )
    ...
    def _generate_large_data(self, nb_runs=1000):
        ...
        for _ in range(nb_runs):
            ...
            mongo_run.update(
                push__latest_metrics=MongoLatestMetric(
                    **{
                        "key": "mkey_0",
                        "value": current_run,
                        "timestamp": 100 * 2,
    ...
    def test_search_runs_returns_expected_results_with_large_experiment(self):
        ...
        run_results = self.store.search_runs(
            [experiment_id], None, ViewType.ALL, max_results=10
        )
    ...
    def test_record_logged_model(self):
        ...
        m = Model(
            artifact_path="model/path", run_id=run_id, flavors={"tf": "flavor body"}
        )
        ...
        self._verify_logged(
            ...
            metrics=[],
            tags=[
                RunTag(MLFLOW_LOGGED_MODELS, json.dumps([m.to_dict(), m2.to_dict()]))
            ],
        )
        m3 = Model(
